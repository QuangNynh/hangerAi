...
interface Props {
  ...
  hideText?: boolean;
  showAfterChose?: boolean;
  sx?: SxProps;
}
function DragAndDropImg({ name, hideText, showAfterChose, sx }: Props) {
  ...
  useEffect(() => {
    ...
    const dragElement = document.getElementById(`${name}-file-image-input`);
  ...
  return (
    ...
    >
      <input
        ...
        style={{ display: 'none' }}
        type="file"
        id={`${name}-hidden-file-input`}
      />
      <Box
        sx={{
          width: '100%',
          textAlign: 'center',
          display: selectImage && showAfterChose ? 'none' : 'block',
        }}
      >
        <Typography
          sx={{
      ...
      <CenterBox
        ...
        id={`${name}-file-image-input`}

        // htmlFor={'hidden-file-input'}
      >
        {showAfterChose && selectImage && (
